function L(n){return btoa(String.fromCharCode(...n)).replace(/\+/g,"-").replace(/\//g,"_").replace(/=/g,"")}function U(n){let e=n.replace(/-/g,"+").replace(/_/g,"/").padEnd(n.length+(4-n.length%4)%4,"="),t=atob(e),a=new Uint8Array(t.length);for(let r=0;r<t.length;r++)a[r]=t.charCodeAt(r);return a}var m=new TextEncoder,C=new TextDecoder;async function b(n,e){let t={alg:"HS256",typ:"JWT"},a=L(m.encode(JSON.stringify(t))),r=L(m.encode(JSON.stringify(n))),s=`${a}.${r}`,i=await crypto.subtle.importKey("raw",m.encode(e),{name:"HMAC",hash:"SHA-256"},!1,["sign"]),o=await crypto.subtle.sign("HMAC",i,m.encode(s)),E=L(new Uint8Array(o));return`${s}.${E}`}async function w(n,e){try{let t=n.split(".");if(t.length!==3)return{valid:!1,error:"Invalid token format"};let[a,r,s]=t;if(!a||!r||!s)return{valid:!1,error:"Invalid token format"};let i=`${a}.${r}`,o=await crypto.subtle.importKey("raw",m.encode(e),{name:"HMAC",hash:"SHA-256"},!1,["verify"]),E=U(s);if(!await crypto.subtle.verify("HMAC",o,E,m.encode(i)))return{valid:!1,error:"Invalid signature"};let T=C.decode(U(r)),d=JSON.parse(T),l=Math.floor(Date.now()/1e3);return d.exp&&d.exp<l?{valid:!1,error:"Token expired"}:{valid:!0,payload:d}}catch(t){return{valid:!1,error:t instanceof Error?t.message:"Verification failed"}}}function O(n){try{let e=n.split(".");if(e.length!==3)return null;let t=e[1];if(!t)return null;let a=C.decode(U(t));return JSON.parse(a)}catch{return null}}var k=3600,I=new WeakMap;async function v(n){let e=I.get(n);if(e)return e;let t=await n.query("SELECT value FROM auth.config WHERE key = 'jwt_secret'");if(t.rows.length>0&&t.rows[0])return I.set(n,t.rows[0].value),t.rows[0].value;let a=new Uint8Array(32);crypto.getRandomValues(a);let r=Array.from(a,s=>s.toString(16).padStart(2,"0")).join("");return await n.query("INSERT INTO auth.config (key, value) VALUES ($1, $2) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value",["jwt_secret",r]),I.set(n,r),r}async function p(n,e,t,a=k){let r=await v(n),s=Math.floor(Date.now()/1e3),i={sub:e.id,aud:"authenticated",role:e.role,email:e.email||void 0,session_id:t,iat:s,exp:s+a,user_metadata:e.user_metadata||{},app_metadata:e.app_metadata||{}};return b(i,r)}async function A(n,e){let t=await v(n);return w(e,t)}async function J(n,e,t,a,r=k){let s=await p(n,e,t,r),i=Math.floor(Date.now()/1e3);return{accessToken:s,refreshToken:a,expiresIn:r,expiresAt:i+r}}function V(n){return O(n)?.sub||null}function D(n){return O(n)?.session_id||null}var F=`
...
`;function R(n){return n.replace(/'/g,"''")}function K(n,e,t){let a=JSON.stringify({sub:n,role:e,email:t,aud:"authenticated"}),r=R(n),s=R(e),i=R(t),o=R(a);return`
    SET ROLE ${s};
...
`;var N=3600;function c(n){return{id:n.id,aud:n.aud,role:n.role,email:n.email,email_confirmed_at:n.email_confirmed_at||void 0,phone:n.phone||void 0,phone_confirmed_at:n.phone_confirmed_at||void 0,confirmed_at:n.email_confirmed_at||n.phone_confirmed_at||void 0,last_sign_in_at:n.last_sign_in_at||void 0,app_metadata:n.raw_app_meta_data||{},user_metadata:n.raw_user_meta_data||{},created_at:n.created_at,updated_at:n.updated_at}}function _(n,e,t){return{message:n,status:e,code:t}}var $=class{db;initPromise=null;subscriptions=new Map;currentSession=null;constructor(e){this.db=e}async initialize(){this.initPromise??=this.db.exec(F),await this.initPromise}emitAuthStateChange(e,t){this.currentSession=t;for(let a of this.subscriptions.values())try{a(e,t)}catch(r){console.error("Auth state change callback error:",r)}}onAuthStateChange(e){let t=crypto.randomUUID();return this.subscriptions.set(t,e),queueMicrotask(()=>{e("INITIAL_SESSION",this.currentSession)}),{id:t,callback:e,unsubscribe:()=>{this.subscriptions.delete(t)}}}async signInAndCreateSession(e){let t=c(e),a=await this.createSession(e);return this.emitAuthStateChange("SIGNED_IN",a),{data:{user:t,session:a},error:null}}async signUp(e,t,a){await this.initialize(),await this.db.exec("RESET ROLE");try{if((await this.db.query("SELECT * FROM auth.users WHERE email = $1",[e])).rows.length>0)return{data:{user:null,session:null},error:_("User already registered",400,"user_already_exists")};let s=a?.data?JSON.stringify(a.data):"{}",o=(await this.db.query("SELECT * FROM auth.create_user($1, $2, $3::jsonb)",[e,t,s])).rows[0];return o?this.signInAndCreateSession(o):{data:{user:null,session:null},error:_("Failed to create user",500,"user_creation_failed")}}catch(r){let s=r instanceof Error?r.message:"Sign up failed";return{data:{user:null,session:null},error:_(s,500,"sign_up_failed")}}}async signInWithPassword(e,t){await this.initialize(),await this.db.exec("RESET ROLE");try{let r=(await this.db.query("SELECT * FROM auth.verify_user_credentials($1, $2)",[e,t])).rows[0];return!r||!r.id?{data:{user:null,session:null},error:_("Invalid login credentials",400,"invalid_credentials")}:this.signInAndCreateSession(r)}catch(a){let r=a instanceof Error?a.message:"Sign in failed";return{data:{user:null,session:null},error:_(r,500,"sign_in_failed")}}}async createSession(e){let a=(await this.db.query("SELECT * FROM auth.create_session($1)",[e.id])).rows[0];if(!a)throw new Error("Failed to create session");let s=(await this.db.query("SELECT * FROM auth.create_refresh_token($1, $2)",[e.id,a.id])).rows[0];if(!s)throw new Error("Failed to create refresh token");let i=c(e);return{access_token:await p(this.db,i,a.id,N),token_type:"bearer",expires_in:N,expires_at:Math.floor(Date.now()/1e3)+N,refresh_token:s.token,user:i}}async refreshSession(e){await this.initialize();try{let a=(await this.db.query("SELECT * FROM auth.refresh_token($1)",[e])).rows[0];if(!a||!a.new_token)return{data:{user:null,session:null},error:_("Invalid refresh token",401,"invalid_refresh_token")};let{new_token:r,user_id:s,session_id:i}=a,E=(await this.db.query("SELECT * FROM auth.users WHERE id = $1",[s])).rows[0];if(!E)return{data:{user:null,session:null},error:_("User not found",404,"user_not_found")};let u=c(E),d={access_token:await p(this.db,u,i,N),token_type:"bearer",expires_in:N,expires_at:Math.floor(Date.now()/1e3)+N,refresh_token:r,user:u};return this.emitAuthStateChange("TOKEN_REFRESHED",d),{data:{user:u,session:d},error:null}}catch(t){let a=t instanceof Error?t.message:"Token refresh failed";return{data:{user:null,session:null},error:_(a,500,"refresh_failed")}}}async signOut(e){await this.initialize();try{if(e){let t=D(e);t&&await this.db.query("SELECT auth.sign_out($1::uuid)",[t])}return await this.db.exec("RESET ROLE"),this.emitAuthStateChange("SIGNED_OUT",null),{error:null}}catch(t){let a=t instanceof Error?t.message:"Sign out failed";return{error:_(a,500,"sign_out_failed")}}}async getUser(e){await this.initialize();try{let t=await A(this.db,e);if(!t.valid||!t.payload)return{data:{user:null},error:_(t.error||"Invalid token",401,"invalid_token")};let r=(await this.db.query("SELECT * FROM auth.users WHERE id = $1",[t.payload.sub])).rows[0];return r?{data:{user:c(r)},error:null}:{data:{user:null},error:_("User not found",404,"user_not_found")}}catch(t){let a=t instanceof Error?t.message:"Get user failed";return{data:{user:null},error:_(a,500,"get_user_failed")}}}async updateUser(e,t){await this.initialize();try{let a=await A(this.db,e);if(!a.valid||!a.payload)return{data:{user:null,session:null},error:_(a.error||"Invalid token",401,"invalid_token")};let r=a.payload.sub,s=[],i=[],o=1;if(t.email&&(s.push(`email = $${o}`),i.push(t.email),o++),t.password&&(s.push(`encrypted_password = auth.hash_password($${o})`),i.push(t.password),o++),t.data&&(s.push(`raw_user_meta_data = raw_user_meta_data || $${o}::jsonb`),i.push(JSON.stringify(t.data)),o++),s.length===0){let h=(await this.db.query("SELECT * FROM auth.users WHERE id = $1",[r])).rows[0];return h?{data:{user:c(h),session:this.currentSession},error:null}:{data:{user:null,session:null},error:_("User not found",404,"user_not_found")}}s.push("updated_at = NOW()"),i.push(r);let u=(await this.db.query(`UPDATE auth.users SET ${s.join(", ")} WHERE id = $${o} RETURNING *`,i)).rows[0];if(!u)return{data:{user:null,session:null},error:_("User not found",404,"user_not_found")};let T=c(u),d=this.currentSession;if(d){let l=await p(this.db,T,a.payload.session_id,N);d={...d,access_token:l,user:T}}return this.emitAuthStateChange("USER_UPDATED",d),{data:{user:T,session:d},error:null}}catch(a){let r=a instanceof Error?a.message:"Update user failed";return{data:{user:null,session:null},error:_(r,500,"update_user_failed")}}}async adminListUsers(e=1,t=50){await this.initialize();let a=(e-1)*t,[r,s]=await Promise.all([this.db.query("SELECT * FROM auth.users ORDER BY created_at DESC LIMIT $1 OFFSET $2",[t,a]),this.db.query("SELECT count(*)::int AS total FROM auth.users")]);return{users:r.rows.map(c),total:s.rows[0]?.total??0}}async adminGetUser(e){await this.initialize();let a=(await this.db.query("SELECT * FROM auth.users WHERE id = $1",[e])).rows[0];return a?c(a):null}async adminCreateUser(e){await this.initialize();let t=JSON.stringify(e.user_metadata??{}),a=JSON.stringify(e.app_metadata??{}),r=e.email_confirm!==!1?"NOW()":"NULL",i=(await this.db.query(`INSERT INTO auth.users (email, phone, encrypted_password, raw_user_meta_data, raw_app_meta_data, email_confirmed_at, confirmed_at, aud, role)
       VALUES ($1, $2, COALESCE(auth.hash_password($3), ''), $4::jsonb, $5::jsonb, ${r}, ${r}, 'authenticated', 'authenticated')
       RETURNING *`,[e.email??null,e.phone??null,e.password??null,t,a])).rows[0];if(!i)throw new Error("Failed to create user");return c(i)}async adminUpdateUser(e,t){await this.initialize();let a=[],r=[],s=1;if(t.email!==void 0&&(a.push(`email = $${s++}`),r.push(t.email)),t.phone!==void 0&&(a.push(`phone = $${s++}`),r.push(t.phone)),t.password&&(a.push(`encrypted_password = auth.hash_password($${s++})`),r.push(t.password)),t.user_metadata&&(a.push(`raw_user_meta_data = raw_user_meta_data || $${s++}::jsonb`),r.push(JSON.stringify(t.user_metadata))),t.app_metadata&&(a.push(`raw_app_meta_data = raw_app_meta_data || $${s++}::jsonb`),r.push(JSON.stringify(t.app_metadata))),t.ban_duration==="none"?a.push("banned_until = NULL"):t.ban_duration&&(a.push(`banned_until = NOW() + $${s++}::interval`),r.push(t.ban_duration)),t.email_confirm&&a.push("email_confirmed_at = COALESCE(email_confirmed_at, NOW())"),a.length===0)return this.adminGetUser(e);a.push("updated_at = NOW()"),r.push(e);let o=(await this.db.query(`UPDATE auth.users SET ${a.join(", ")} WHERE id = $${s} RETURNING *`,r)).rows[0];return o?c(o):null}async adminDeleteUser(e){await this.initialize(),await this.db.query("DELETE FROM auth.users WHERE id = $1",[e])}getSession(){return this.currentSession}setSession(e){this.currentSession=e,e&&this.emitAuthStateChange("SIGNED_IN",e)}async verifyToken(e){return A(this.db,e)}};var g=class{store=new Map;async put(e,t,a){this.store.set(e,{data:t,metadata:a})}async get(e){return this.store.get(e)??null}async delete(e){return this.store.delete(e)}async deleteByPrefix(e){let t=0;for(let a of this.store.keys())a.startsWith(e)&&(this.store.delete(a),t++);return t}async exists(e){return this.store.has(e)}async copy(e,t){let a=this.store.get(e);return a?(this.store.set(t,{data:new Uint8Array(a.data),metadata:{...a.metadata}}),!0):!1}};var P=`
...
`;function M(n){return n.replace(/\+/g,"-").replace(/\//g,"_").replace(/=+$/,"")}function x(n){let e=n.replace(/-/g,"+").replace(/_/g,"/");for(;e.length%4;)e+="=";return e}var G=class{db;backend;initPromise=null;constructor(e,t){this.db=e,this.backend=t??new g}async initialize(){this.initPromise??=this.db.exec(P),await this.initPromise}getBackend(){return this.backend}async listBuckets(){return await this.initialize(),(await this.db.query("SELECT * FROM storage.buckets ORDER BY name")).rows}async getBucket(e){return await this.initialize(),(await this.db.query("SELECT * FROM storage.buckets WHERE id = $1",[e])).rows[0]??null}async createBucket(e){await this.initialize();let t=e.id??e.name,r=(await this.db.query(`INSERT INTO storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
       ...
       RETURNING *`,[t,e.name,e.public??!1,e.file_size_limit??null,e.allowed_mime_types??null])).rows[0];if(!r)throw new Error("Failed to create bucket");return r}async updateBucket(e,t){await this.initialize();let a=[],r=[],s=1;if(t.public!==void 0&&(a.push(`public = $${s++}`),r.push(t.public)),t.file_size_limit!==void 0&&(a.push(`file_size_limit = $${s++}`),r.push(t.file_size_limit)),t.allowed_mime_types!==void 0&&(a.push(`allowed_mime_types = $${s++}`),r.push(t.allowed_mime_types)),a.length===0){let E=await this.getBucket(e);if(!E)throw new Error("Bucket not found");return E}a.push("updated_at = now()"),r.push(e);let o=(await this.db.query(`UPDATE storage.buckets SET ${a.join(", ")} WHERE id = $${s} RETURNING *`,r)).rows[0];if(!o)throw new Error("Bucket not found");return o}async emptyBucket(e){await this.initialize(),await this.db.query("DELETE FROM storage.objects WHERE bucket_id = $1",[e]),await this.backend.deleteByPrefix(`${e}/`)}async deleteBucket(e){await this.initialize();let t=await this.db.query("SELECT count(*)::text as count FROM storage.objects WHERE bucket_id = $1",[e]);if(t.rows[0]&&parseInt(t.rows[0].count,10)>0)throw new Error("Bucket not empty");await this.db.query("DELETE FROM storage.buckets WHERE id = $1",[e])}async uploadObject(e,t,a,r,s){await this.initialize();let i=await this.getBucket(e);if(!i)throw new Error("Bucket not found");if(i.file_size_limit&&a.byteLength>i.file_size_limit)throw new Error(`File size ${a.byteLength} exceeds bucket limit of ${i.file_size_limit}`);if(i.allowed_mime_types&&i.allowed_mime_types.length>0&&!i.allowed_mime_types.some(h=>h.endsWith("/*")?r.startsWith(h.slice(0,-1)):r===h))throw new Error(`MIME type ${r} is not allowed in this bucket`);let o={eTag:`"${await this.computeETag(a)}"`,size:a.byteLength,mimetype:r,cacheControl:s?.cacheControl??"max-age=3600",lastModified:new Date().toISOString(),contentLength:a.byteLength,httpStatusCode:200},E=s?.upsert??!1,u;E?u=await this.db.query(`INSERT INTO storage.objects (bucket_id, name, owner_id, metadata, user_metadata, version)
         ...
         RETURNING *`,[e,t,s?.ownerId??null,JSON.stringify(o),s?.userMetadata?JSON.stringify(s.userMetadata):null]):u=await this.db.query(`INSERT INTO storage.objects (bucket_id, name, owner_id, metadata, user_metadata, version)
         VALUES ($1, $2, $3, $4, $5, gen_random_uuid()::text)
         RETURNING *`,[e,t,s?.ownerId??null,JSON.stringify(o),s?.userMetadata?JSON.stringify(s.userMetadata):null]);let T=u.rows[0];if(!T)throw new Error("Failed to create object");let d=`${e}/${t}`;return await this.backend.put(d,a,{contentType:r,size:a.byteLength,cacheControl:s?.cacheControl}),T}async downloadObject(e,t){await this.initialize();let r=(await this.db.query(`UPDATE storage.objects
       ...
       RETURNING *`,[e,...t]);for(let s of r.rows)await this.backend.delete(`${e}/${s.name}`);return r.rows}async listObjects(e,t){await this.initialize();let a=t?.prefix??"",r=t?.limit??100,s=t?.offset??0,i=t?.sortBy?.column??"name",o=t?.sortBy?.order?.toLowerCase()==="desc"?"DESC":"ASC",u=["name","created_at","updated_at","last_accessed_at"].includes(i)?i:"name",T=t?.search??"",d=a.split("/").filter(Boolean).length+1;return(await this.db.query("SELECT * FROM storage.search($1, $2, $3, $8, $4, $5, $6, $7)",[a,e,r,s,T,u,o.toLowerCase(),d])).rows}async moveObject(e,t,a,r){await this.initialize();let s=r??e;if((await this.db.query(`UPDATE storage.objects
       SET bucket_id = $3, name = $4, updated_at = now()
       WHERE bucket_id = $1 AND name = $2
       RETURNING *`,[e,t,s,a])).rows.length===0)throw new Error("Object not found");let o=`${e}/${t}`,E=`${s}/${a}`;await this.backend.copy(o,E)&&await this.backend.delete(o)}async copyObject(e,t,a,r){await this.initialize();let s=r??e,o=(await this.db.query("SELECT * FROM storage.objects WHERE bucket_id = $1 AND name = $2",[e,t])).rows[0];if(!o)throw new Error("Object not found");if(!(await this.db.query(`INSERT INTO storage.objects (bucket_id, name, owner_id, metadata, user_metadata, version)
       VALUES ($1, $2, $3, $4, $5, gen_random_uuid()::text)
       ON CONFLICT (bucket_id, name)
       DO UPDATE SET
         ...
         updated_at = now(),
         version = gen_random_uuid()::text
       RETURNING *`,[s,a,o.owner_id,JSON.stringify(o.metadata),o.user_metadata?JSON.stringify(o.user_metadata):null])).rows[0])throw new Error("Failed to copy object");let T=`${e}/${t}`,d=`${s}/${a}`;return await this.backend.copy(T,d),`${s}/${a}`}async createSignedUrl(e,t,a){await this.initialize();let r={bucket_id:e,object_name:t,exp:Math.floor(Date.now()/1e3)+a},i=(await this.db.query("SELECT auth.get_signing_key()")).rows[0]?.get_signing_key;if(!i)throw new Error("Signing key not available");let o=new TextEncoder,E=await crypto.subtle.importKey("raw",o.encode(i),{name:"HMAC",hash:"SHA-256"},!1,["sign"]),u=JSON.stringify(r),T=await crypto.subtle.sign("HMAC",E,o.encode(u)),d=M(btoa(u)),l=M(btoa(String.fromCharCode(...new Uint8Array(T))));return`${d}.${l}`}async verifySignedUrl(e){let t=e.split(".");if(t.length!==2)return null;let[a,r]=t;try{let s=atob(x(a??"")),i=JSON.parse(s);if(i.exp<Math.floor(Date.now()/1e3))return null;let o=await this.db.query("SELECT auth.get_signing_key()");if(!o.rows[0])return null;let E=new TextEncoder,u=await crypto.subtle.importKey("raw",E.encode(o.rows[0].get_signing_key),{name:"HMAC",hash:"SHA-256"},!1,["verify"]),T=Uint8Array.from(atob(x(r??"")),l=>l.charCodeAt(0));return await crypto.subtle.verify("HMAC",u,T,E.encode(s))?i:null}catch{return null}}async computeETag(e){try{let t=e.buffer.slice(e.byteOffset,e.byteOffset+e.byteLength),a=await crypto.subtle.digest("SHA-256",t),r=new Uint8Array(a);return Array.from(r.slice(0,8)).map(s=>s.toString(16).padStart(2,"0")).join("")}catch{return Math.random().toString(36).slice(2,18)}}};var y=(r=>(r[r.CRITICAL=0]="CRITICAL",r[r.HIGH=1]="HIGH",r[r.MEDIUM=2]="MEDIUM",r[r.LOW=3]="LOW",r))(y||{});var X={1:0,2:1,3:2},B=4,q=1e3,S=class{queues;heads;maxSize;agingThresholdMs;_size=0;lastAgingRun=0;constructor(e=1e3,t=5e3){this.maxSize=e,this.agingThresholdMs=t,this.queues=[[],[],[],[]],this.heads=[0,0,0,0]}enqueue(e){if(e.priority<0||e.priority>3)throw new Error(`Invalid priority: ${e.priority}`);if(this._size>=this.maxSize)throw new Error(`Queue is full (size: ${this._size}, max: ${this.maxSize})`);this.queues[e.priority].push(e),this._size++}dequeue(){let e=Date.now();e-this.lastAgingRun>q&&(this.applyAging(e),this.lastAgingRun=e);for(let t=0;t<B;t++){let a=t,r=this.queues[a],s=this.heads[a];if(s<r.length){let i=r[s]??null;return r[s]=null,this.heads[a]++,this._size--,this.heads[a]>r.length/2&&(this.queues[a]=r.slice(this.heads[a]),this.heads[a]=0),i}}return null}applyAging(e){for(let t of[1,2,3]){let a=this.queues[t],r=this.heads[t];if(r>=a.length)continue;let s=!1,i=this.queues[X[t]];for(let o=r;o<a.length;o++){let E=a[o];E!==void 0&&e-E.enqueuedAt>this.agingThresholdMs&&(E.priority=X[t],i.push(E),a[o]=null,s=!0)}if(s){let o=[];for(let E=r;E<a.length;E++){let u=a[E];u!=null&&o.push(u)}this.queues[t]=o,this.heads[t]=0}}}size(){return this._size}clear(){let e=[];for(let t=0;t<B;t++){let a=t,r=this.queues[a],s=this.heads[a];for(let i=s;i<r.length;i++){let o=r[i];o!==void 0&&e.push(o)}this.queues[a]=[],this.heads[a]=0}return this._size=0,e}sizeByPriority(){return{0:Math.max(0,this.queues[0].length-this.heads[0]),1:Math.max(0,this.queues[1].length-this.heads[1]),2:Math.max(0,this.queues[2].length-this.heads[2]),3:Math.max(0,this.queues[3].length-this.heads[3])}}};var W=1e3,Q=5e3,j=5e3;function f(n){return n instanceof Error?n:new Error(String(n))}var H=class n{db;queue;running=!1;config;wakeUp=null;processQueueDone=Promise.resolve();resolveProcessQueueDone=null;totalEnqueued=0;totalDequeued=0;totalTimedOut=0;totalErrors=0;waitTimeSum=0;waitTimeCount=0;nextQueryId=0;get pglite(){return this.db}constructor(e,t={}){this.db=e;let a=t.maxQueueSize??W,r=t.agingThresholdMs??j;this.queue=new S(a,r),this.config={maxQueueSize:a,defaultTimeout:t.defaultTimeout??Q,agingThresholdMs:r}}static async create(e,t){let a=new n(e,t);return await a.start(),a}async start(){if(this.running)throw new Error("Pooler already started");this.running=!0,this.processQueueDone=new Promise(e=>{this.resolveProcessQueueDone=e}),setTimeout(()=>{this.processQueue().catch(e=>{console.error("Queue processor error:",e),this.running=!1,this.resolveProcessQueueDone?.()})},0),await new Promise(e=>setTimeout(e,0))}async stop(){if(!this.running)return;this.running=!1,this.wakeUp?.();let e=this.queue.clear();for(let t of e)t.reject(new Error("Pooler stopped"));await this.processQueueDone}async query(e,t,a=2,r){if(!this.running)throw new Error("Pooler is not running");return new Promise((s,i)=>{let o={kind:"sql",id:String(this.nextQueryId++),sql:e,params:t??[],priority:a,enqueuedAt:Date.now(),resolve:s,reject:i,timeoutMs:r??this.config.defaultTimeout};try{this.queue.enqueue(o),this.totalEnqueued++,this.wakeUp?.()}catch(E){i(f(E))}})}async transaction(e,t=2){if(!this.running)throw new Error("Pooler is not running");return new Promise((a,r)=>{let s={kind:"transaction",id:String(this.nextQueryId++),priority:t,enqueuedAt:Date.now(),resolve:a,reject:r,timeoutMs:this.config.defaultTimeout,transactionFn:e};try{this.queue.enqueue(s),this.totalEnqueued++,this.wakeUp?.()}catch(i){r(f(i))}})}metrics(){let e=this.waitTimeCount>0?this.waitTimeSum/this.waitTimeCount:0;return{totalEnqueued:this.totalEnqueued,totalDequeued:this.totalDequeued,totalTimedOut:this.totalTimedOut,totalErrors:this.totalErrors,currentSize:this.queue.size(),avgWaitTimeMs:e,sizeByPriority:this.queue.sizeByPriority()}}async[Symbol.asyncDispose](){await this.stop()}async processQueue(){for(;this.running;){let e=this.queue.dequeue();if(!e){await new Promise(t=>{this.wakeUp=t,setTimeout(()=>t(),1)}),this.wakeUp=null;continue}this.totalDequeued++,this.waitTimeSum+=Date.now()-e.enqueuedAt,this.waitTimeCount++;try{if(e.kind==="transaction"){let t=await this.runTransaction(e.transactionFn);e.resolve(t)}else{let t=await this.executeWithTimeout(e);e.resolve(t)}}catch(t){this.totalErrors++;let a=f(t);a.message==="Query timeout"&&this.totalTimedOut++,e.reject(a)}}this.resolveProcessQueueDone?.()}async runTransaction(e){return this.db.transaction(async t=>e((r,s)=>t.query(r,s)))}async executeWithTimeout(e){let t=e.timeoutMs??this.config.defaultTimeout,a=null,r=new Promise((i,o)=>{a=setTimeout(()=>{o(new Error("Query timeout"))},t)}),s=this.db.query(e.sql,[...e.params??[]]).finally(()=>{a!==null&&clearTimeout(a)});return Promise.race([s,r])}};export{b as a,w as b,O as c,p as d,A as e,J as f,V as g,D as h,F as i,K as j,ee as k,$ as l,g as m,P as n,G as o,y as p,S as q,H as r};
//# sourceMappingURL=chunk-2DKMOA3W.js.map
