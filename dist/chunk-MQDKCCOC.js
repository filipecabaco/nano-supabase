function O(n){return btoa(String.fromCharCode(...n)).replace(/\+/g,"-").replace(/\//g,"_").replace(/=/g,"")}function U(n){let e=n.replace(/-/g,"+").replace(/_/g,"/").padEnd(n.length+(4-n.length%4)%4,"="),t=atob(e),a=new Uint8Array(t.length);for(let r=0;r<t.length;r++)a[r]=t.charCodeAt(r);return a}var g=new TextEncoder,k=new TextDecoder;async function v(n,e){let t={alg:"HS256",typ:"JWT"},a=O(g.encode(JSON.stringify(t))),r=O(g.encode(JSON.stringify(n))),s=`${a}.${r}`,i=await crypto.subtle.importKey("raw",g.encode(e),{name:"HMAC",hash:"SHA-256"},!1,["sign"]),o=await crypto.subtle.sign("HMAC",i,g.encode(s)),u=O(new Uint8Array(o));return`${s}.${u}`}async function D(n,e){try{let t=n.split(".");if(t.length!==3)return{valid:!1,error:"Invalid token format"};let[a,r,s]=t;if(!a||!r||!s)return{valid:!1,error:"Invalid token format"};let i=`${a}.${r}`,o=await crypto.subtle.importKey("raw",g.encode(e),{name:"HMAC",hash:"SHA-256"},!1,["verify"]),u=U(s);if(!await crypto.subtle.verify("HMAC",o,u,g.encode(i)))return{valid:!1,error:"Invalid signature"};let _=k.decode(U(r)),d=JSON.parse(_),T=Math.floor(Date.now()/1e3);return d.exp&&d.exp<T?{valid:!1,error:"Token expired"}:{valid:!0,payload:d}}catch(t){return{valid:!1,error:t instanceof Error?t.message:"Verification failed"}}}function I(n){try{let e=n.split(".");if(e.length!==3)return null;let t=e[1];if(!t)return null;let a=k.decode(U(t));return JSON.parse(a)}catch{return null}}var F=3600,y=new WeakMap;async function $(n){let e=y.get(n);if(e)return e;let t=await n.query("SELECT value FROM auth.config WHERE key = 'jwt_secret'");if(t.rows.length>0&&t.rows[0])return y.set(n,t.rows[0].value),t.rows[0].value;let a=new Uint8Array(32);crypto.getRandomValues(a);let r=Array.from(a,s=>s.toString(16).padStart(2,"0")).join("");return await n.query("INSERT INTO auth.config (key, value) VALUES ($1, $2) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value",["jwt_secret",r]),y.set(n,r),r}async function A(n,e,t,a=F){let r=await $(n),s=Math.floor(Date.now()/1e3),i={sub:e.id,aud:"authenticated",role:e.role,email:e.email||void 0,session_id:t,iat:s,exp:s+a,user_metadata:e.user_metadata||{},app_metadata:e.app_metadata||{}};return v(i,r)}async function N(n,e){let t=await $(n);return D(e,t)}async function re(n,e,t,a,r=F){let s=await A(n,e,t,r),i=Math.floor(Date.now()/1e3);return{accessToken:s,refreshToken:a,expiresIn:r,expiresAt:i+r}}function se(n){return I(n)?.sub||null}function P(n){return I(n)?.session_id||null}var M=`
...
`,j=new Set(["anon","authenticated","service_role","supabase_admin","postgres"]);function f(n){return n.replace(/'/g,"''")}function ne(n,e,t){if(!j.has(e))throw new Error(`Invalid role: ${e}`);let a=JSON.stringify({sub:n,role:e,email:t,aud:"authenticated"}),r=f(n),s=f(t),i=f(a);return`
    SET ROLE ${e};
