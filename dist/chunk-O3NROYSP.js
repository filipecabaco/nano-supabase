function L(n){return btoa(String.fromCharCode(...n)).replace(/\+/g,"-").replace(/\//g,"_").replace(/=/g,"")}function I(n){let e=n.replace(/-/g,"+").replace(/_/g,"/").padEnd(n.length+(4-n.length%4)%4,"="),t=atob(e),a=new Uint8Array(t.length);for(let r=0;r<t.length;r++)a[r]=t.charCodeAt(r);return a}var g=new TextEncoder,v=new TextDecoder;async function k(n,e){let t={alg:"HS256",typ:"JWT"},a=L(g.encode(JSON.stringify(t))),r=L(g.encode(JSON.stringify(n))),s=`${a}.${r}`,i=await crypto.subtle.importKey("raw",g.encode(e),{name:"HMAC",hash:"SHA-256"},!1,["sign"]),o=await crypto.subtle.sign("HMAC",i,g.encode(s)),u=L(new Uint8Array(o));return`${s}.${u}`}async function D(n,e){try{let t=n.split(".");if(t.length!==3)return{valid:!1,error:"Invalid token format"};let[a,r,s]=t;if(!a||!r||!s)return{valid:!1,error:"Invalid token format"};let i=`${a}.${r}`,o=await crypto.subtle.importKey("raw",g.encode(e),{name:"HMAC",hash:"SHA-256"},!1,["verify"]),u=I(s);if(!await crypto.subtle.verify("HMAC",o,u,g.encode(i)))return{valid:!1,error:"Invalid signature"};let _=v.decode(I(r)),d=JSON.parse(_),T=Math.floor(Date.now()/1e3);return d.exp&&d.exp<T?{valid:!1,error:"Token expired"}:{valid:!0,payload:d}}catch(t){return{valid:!1,error:t instanceof Error?t.message:"Verification failed"}}}function U(n){try{let e=n.split(".");if(e.length!==3)return null;let t=e[1];if(!t)return null;let a=v.decode(I(t));return JSON.parse(a)}catch{return null}}var F=3600,y=new WeakMap;async function $(n){let e=y.get(n);if(e)return e;let t=await n.query("SELECT value FROM auth.config WHERE key = 'jwt_secret'");if(t.rows.length>0&&t.rows[0])return y.set(n,t.rows[0].value),t.rows[0].value;let a=new Uint8Array(32);crypto.getRandomValues(a);let r=Array.from(a,s=>s.toString(16).padStart(2,"0")).join("");return await n.query("INSERT INTO auth.config (key, value) VALUES ($1, $2) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value",["jwt_secret",r]),y.set(n,r),r}async function A(n,e,t,a=F){let r=await $(n),s=Math.floor(Date.now()/1e3),i={sub:e.id,aud:"authenticated",role:e.role,email:e.email||void 0,session_id:t,iat:s,exp:s+a,user_metadata:e.user_metadata||{},app_metadata:e.app_metadata||{}};return k(i,r)}async function N(n,e){let t=await $(n);return D(e,t)}async function ie(n,e,t,a,r=F){let s=await A(n,e,t,r),i=Math.floor(Date.now()/1e3);return{accessToken:s,refreshToken:a,expiresIn:r,expiresAt:i+r}}function ne(n){return U(n)?.sub||null}function P(n){return U(n)?.session_id||null}var M=`
...
`,j=new Set(["anon","authenticated","service_role","supabase_admin","postgres"]);function f(n){return n.replace(/'/g,"''")}function z(n,e,t,a={}){if(!j.has(e))throw new Error(`Invalid role: ${e}`);let r=JSON.stringify({...a,sub:n,role:e,email:t,aud:"authenticated"}),s=f(n),i=f(t),o=f(r);return`
    SET ROLE ${e};
    SELECT set_config('request.jwt.claim.sub', '${s}', false);
    SELECT set_config('request.jwt.claim.role', '${e}', false);
...
`;async function ue(n,e){let{role:t,claims:a={}}=e,{sub:r="",email:s="",...i}=a,o=z(r,t,s,i);return await n.exec(o),{async reset(){await n.exec(Y)}}}var p=3600;function J(n,e){if(n.length!==e.length)return!1;let t=0;for(let a=0;a<n.length;a++)t|=n.charCodeAt(a)^e.charCodeAt(a);return t===0}function R(n){return Array.from(n,e=>e.toString(16).padStart(2,"0")).join("")}function c(n){return{id:n.id,aud:n.aud,role:n.role,email:n.email,email_confirmed_at:n.email_confirmed_at||void 0,phone:n.phone||void 0,phone_confirmed_at:n.phone_confirmed_at||void 0,confirmed_at:n.email_confirmed_at||n.phone_confirmed_at||void 0,last_sign_in_at:n.last_sign_in_at||void 0,app_metadata:n.raw_app_meta_data||{},user_metadata:n.raw_user_meta_data||{},banned_until:n.banned_until,created_at:n.created_at,updated_at:n.updated_at}}function m(n,e,t){return{message:n,status:e,code:t}}function l(n,e,t){return{data:{user:null,session:null},error:m(n,e,t)}}var x=class{db;initPromise=null;subscriptions=new Map;currentSession=null;constructor(e){this.db=e}async initialize(){this.initPromise??=this.db.exec(M),await this.initPromise}emitAuthStateChange(e,t){this.currentSession=t;for(let a of this.subscriptions.values())try{a(e,t)}catch(r){console.error("Auth state change callback error:",r)}}onAuthStateChange(e){let t=crypto.randomUUID();return this.subscriptions.set(t,e),queueMicrotask(()=>{e("INITIAL_SESSION",this.currentSession)}),{id:t,callback:e,unsubscribe:()=>{this.subscriptions.delete(t)}}}async signInAndCreateSession(e){let t=c(e),a=await this.createSession(e);return this.emitAuthStateChange("SIGNED_IN",a),{data:{user:t,session:a},error:null}}async signUp(e,t,a){await this.initialize(),await this.db.exec("RESET ROLE");try{if((await this.db.query("SELECT * FROM auth.users WHERE email = $1",[e])).rows.length>0)return l("User already registered",400,"user_already_exists");let s=a?.data?JSON.stringify(a.data):"{}",o=(await this.db.query("SELECT * FROM auth.create_user($1, $2, $3::jsonb)",[e,t,s])).rows[0];return o?(await this.writeAuditLog("signup",o.id,e,"account"),this.signInAndCreateSession(o)):l("Failed to create user",500,"user_creation_failed")}catch(r){let s=r instanceof Error?r.message:"Sign up failed";return l(s,500,"sign_up_failed")}}async signInWithPassword(e,t){await this.initialize(),await this.db.exec("RESET ROLE");try{let r=(await this.db.query("SELECT * FROM auth.verify_user_credentials($1, $2)",[e,t])).rows[0];return r?.id?(await this.writeAuditLog("login",r.id,r.email,"account"),this.signInAndCreateSession(r)):(await this.writeAuditLog("login_failed",null,e,"account"),l("Invalid login credentials",400,"invalid_credentials"))}catch(a){let r=a instanceof Error?a.message:"Sign in failed";return l(r,500,"sign_in_failed")}}async createSession(e){let a=(await this.db.query("SELECT * FROM auth.create_session($1)",[e.id])).rows[0];if(!a)throw new Error("Failed to create session");let s=(await this.db.query("SELECT * FROM auth.create_refresh_token($1, $2)",[e.id,a.id])).rows[0];if(!s)throw new Error("Failed to create refresh token");let i=c(e);return{access_token:await A(this.db,i,a.id,p),token_type:"bearer",expires_in:p,expires_at:Math.floor(Date.now()/1e3)+p,refresh_token:s.token,user:i}}async refreshSession(e){await this.initialize();try{let a=(await this.db.query("SELECT * FROM auth.refresh_token($1)",[e])).rows[0];if(!a?.new_token)return l("Invalid refresh token",401,"invalid_refresh_token");let{new_token:r,user_id:s,session_id:i}=a,u=(await this.db.query("SELECT * FROM auth.users WHERE id = $1",[s])).rows[0];if(!u)return l("User not found",404,"user_not_found");let E=c(u),d={access_token:await A(this.db,E,i,p),token_type:"bearer",expires_in:p,expires_at:Math.floor(Date.now()/1e3)+p,refresh_token:r,user:E};return this.emitAuthStateChange("TOKEN_REFRESHED",d),{data:{user:E,session:d},error:null}}catch(t){let a=t instanceof Error?t.message:"Token refresh failed";return l(a,500,"refresh_failed")}}async signOut(e){await this.initialize();try{let t=null;if(e){let a=P(e);if(a){try{t=(await this.db.query("SELECT user_id FROM auth.sessions WHERE id = $1::uuid",[a])).rows[0]?.user_id??null}catch{}await this.db.query("SELECT auth.sign_out($1::uuid)",[a])}}return await this.db.exec("RESET ROLE"),t&&await this.writeAuditLog("logout",t,null,"account"),this.emitAuthStateChange("SIGNED_OUT",null),{error:null}}catch(t){let a=t instanceof Error?t.message:"Sign out failed";return{error:m(a,500,"sign_out_failed")}}}async getUser(e){await this.initialize();try{let t=await N(this.db,e);if(!t.valid||!t.payload)return{data:{user:null},error:m(t.error||"Invalid token",401,"invalid_token")};let r=(await this.db.query("SELECT * FROM auth.users WHERE id = $1",[t.payload.sub])).rows[0];return r?{data:{user:c(r)},error:null}:{data:{user:null},error:m("User not found",404,"user_not_found")}}catch(t){let a=t instanceof Error?t.message:"Get user failed";return{data:{user:null},error:m(a,500,"get_user_failed")}}}async verifyNonce(e,t){let a=await this.db.query("SELECT reauthentication_token, reauthentication_sent_at FROM auth.users WHERE id = $1",[e]),r=a.rows[0]?.reauthentication_token;if(!r||r.length!==t.length||!J(r,t))return l("Invalid nonce",422,"invalid_nonce");let s=new Date(a.rows[0]?.reauthentication_sent_at??0).getTime();return Date.now()-s>600*1e3?l("Reauthentication token expired",401,"nonce_expired"):(await this.db.query("UPDATE auth.users SET reauthentication_token = NULL, updated_at = NOW() WHERE id = $1",[e]),null)}async updateUser(e,t){await this.initialize();try{let a=await N(this.db,e);if(!a.valid||!a.payload)return l(a.error||"Invalid token",401,"invalid_token");let r=a.payload.sub;if(t.nonce!==void 0){let T=await this.verifyNonce(r,t.nonce);if(T)return T}let s=[],i=[],o=1;if(t.email&&(s.push(`email = $${o}`),i.push(t.email),o++),t.password&&(s.push(`encrypted_password = auth.hash_password($${o})`),i.push(t.password),o++),t.data&&(s.push(`raw_user_meta_data = raw_user_meta_data || $${o}::jsonb`),i.push(JSON.stringify(t.data)),o++),s.length===0){let h=(await this.db.query("SELECT * FROM auth.users WHERE id = $1",[r])).rows[0];return h?{data:{user:c(h),session:this.currentSession},error:null}:l("User not found",404,"user_not_found")}s.push("updated_at = NOW()"),i.push(r);let E=(await this.db.query(`UPDATE auth.users SET ${s.join(", ")} WHERE id = $${o} RETURNING *`,i)).rows[0];if(!E)return l("User not found",404,"user_not_found");let _=c(E),d=this.currentSession;if(d){let T=await A(this.db,_,a.payload.session_id,p);d={...d,access_token:T,user:_}}return this.emitAuthStateChange("USER_UPDATED",d),{data:{user:_,session:d},error:null}}catch(a){let r=a instanceof Error?a.message:"Update user failed";return l(r,500,"update_user_failed")}}async adminListUsers(e=1,t=50){await this.initialize();let a=(e-1)*t,[r,s]=await Promise.all([this.db.query("SELECT * FROM auth.users ORDER BY created_at DESC LIMIT $1 OFFSET $2",[t,a]),this.db.query("SELECT count(*)::int AS total FROM auth.users")]);return{users:r.rows.map(c),total:s.rows[0]?.total??0}}async adminGetUser(e){await this.initialize();let a=(await this.db.query("SELECT * FROM auth.users WHERE id = $1",[e])).rows[0];return a?c(a):null}async adminCreateUser(e){await this.initialize();let t=JSON.stringify(e.user_metadata??{}),a=JSON.stringify(e.app_metadata??{}),r=e.email_confirm!==!1?"NOW()":"NULL",i=(await this.db.query(`INSERT INTO auth.users (email, phone, encrypted_password, raw_user_meta_data, raw_app_meta_data, email_confirmed_at, confirmed_at, aud, role)
       VALUES ($1, $2, COALESCE(auth.hash_password($3), ''), $4::jsonb, $5::jsonb, ${r}, ${r}, 'authenticated', 'authenticated')
       RETURNING *`,[e.email??null,e.phone??null,e.password??null,t,a])).rows[0];if(!i)throw new Error("Failed to create user");return c(i)}async adminUpdateUser(e,t){await this.initialize();let a=[],r=[],s=1;if(t.email!==void 0&&(a.push(`email = $${s++}`),r.push(t.email)),t.phone!==void 0&&(a.push(`phone = $${s++}`),r.push(t.phone)),t.password&&(a.push(`encrypted_password = auth.hash_password($${s++})`),r.push(t.password)),t.user_metadata!==void 0&&(a.push(`raw_user_meta_data = raw_user_meta_data || $${s++}::jsonb`),r.push(JSON.stringify(t.user_metadata))),t.app_metadata!==void 0&&(a.push(`raw_app_meta_data = raw_app_meta_data || $${s++}::jsonb`),r.push(JSON.stringify(t.app_metadata))),t.ban_duration==="none"?a.push("banned_until = NULL"):t.ban_duration&&(a.push(`banned_until = NOW() + $${s++}::interval`),r.push(t.ban_duration)),t.email_confirm&&a.push("email_confirmed_at = COALESCE(email_confirmed_at, NOW())"),a.length===0)return this.adminGetUser(e);a.push("updated_at = NOW()"),r.push(e);let o=(await this.db.query(`UPDATE auth.users SET ${a.join(", ")} WHERE id = $${s} RETURNING *`,r)).rows[0];return o?c(o):null}async adminDeleteUser(e){await this.initialize(),await this.db.query("DELETE FROM auth.users WHERE id = $1",[e])}getSession(){return this.currentSession}setSession(e){this.currentSession=e,e&&this.emitAuthStateChange("SIGNED_IN",e)}async writeAuditLog(e,t,a,r){try{let s=JSON.stringify({action:e,actor_id:t,actor_username:a,log_type:r});await this.db.query("INSERT INTO auth.audit_log_entries (id, payload, created_at, ip_address) VALUES (gen_random_uuid(), $1::json, NOW(), '')",[s])}catch{}}async generateOneTimeToken(e,t,a){let r=crypto.getRandomValues(new Uint8Array(32)),s=R(r),i=await crypto.subtle.digest("SHA-256",new TextEncoder().encode(s)),o=R(new Uint8Array(i));return await this.db.query("DELETE FROM auth.one_time_tokens WHERE expires_at < NOW()"),await this.db.query("DELETE FROM auth.one_time_tokens WHERE user_id = $1 AND token_type = $2",[e,t]),await this.db.query("INSERT INTO auth.one_time_tokens (id, user_id, token_type, token_hash, relates_to, expires_at) VALUES (gen_random_uuid(), $1, $2, $3, $4, NOW() + INTERVAL '10 minutes')",[e,t,o,a]),s}async consumeOneTimeToken(e,t){let a=await crypto.subtle.digest("SHA-256",new TextEncoder().encode(e)),r=R(new Uint8Array(a)),i=(await this.db.query("DELETE FROM auth.one_time_tokens WHERE token_hash = $1 AND token_type = $2 AND expires_at > NOW() RETURNING user_id, relates_to, token_type",[r,t])).rows[0];return i?{userId:i.user_id,relatesTo:i.relates_to}:null}async signInAnonymously(){await this.initialize(),await this.db.exec("RESET ROLE");try{let t=(await this.db.query("INSERT INTO auth.users (aud, role, is_anonymous, email_confirmed_at, confirmed_at, created_at, updated_at) VALUES ('authenticated', 'authenticated', true, NOW(), NOW(), NOW(), NOW()) RETURNING *")).rows[0];return t?this.signInAndCreateSession(t):l("Failed to create anonymous user",500,"anonymous_sign_in_failed")}catch(e){let t=e instanceof Error?e.message:"Anonymous sign in failed";return l(t,500,"anonymous_sign_in_failed")}}async sendOtp(e){await this.initialize();let a=(await this.db.query("SELECT * FROM auth.users WHERE email = $1",[e])).rows[0];if(!a&&(a=(await this.db.query("INSERT INTO auth.users (email, aud, role, created_at, updated_at) VALUES ($1, 'authenticated', 'authenticated', NOW(), NOW()) RETURNING *",[e])).rows[0],!a))throw new Error("Failed to create user for OTP");return{token:await this.generateOneTimeToken(a.id,"magiclink",e)}}async sendRecovery(e){await this.initialize();let a=(await this.db.query("SELECT * FROM auth.users WHERE email = $1",[e])).rows[0];return a?{token:await this.generateOneTimeToken(a.id,"recovery",e)}:{token:null}}async verifyOtp(e,t){await this.initialize();let a=t==="recovery"?"recovery":t==="invite"?"invite":"magiclink",r=await this.consumeOneTimeToken(e,a);if(!r)return l("Token has expired or is invalid",403,"otp_expired");let i=(await this.db.query("SELECT * FROM auth.users WHERE id = $1",[r.userId])).rows[0];if(!i)return l("User not found",404,"user_not_found");if(!i.email_confirmed_at){let u=(await this.db.query("UPDATE auth.users SET email_confirmed_at = NOW(), confirmed_at = NOW(), updated_at = NOW() WHERE id = $1 RETURNING *",[i.id])).rows[0];return u?this.signInAndCreateSession(u):l("User not found after update",500,"internal_error")}return this.signInAndCreateSession(i)}async sendInvite(e){await this.initialize();let a=(await this.db.query("SELECT * FROM auth.users WHERE email = $1",[e])).rows[0];if(!a&&(a=(await this.db.query("INSERT INTO auth.users (email, aud, role, invited_at, created_at, updated_at) VALUES ($1, 'authenticated', 'authenticated', NOW(), NOW(), NOW()) RETURNING *",[e])).rows[0],!a))throw new Error("Failed to create user for invite");return{token:await this.generateOneTimeToken(a.id,"invite",e)}}async reauthenticate(e){await this.initialize();let t=await N(this.db,e);if(!t.valid||!t.payload)return{error:m(t.error||"Invalid token",401,"invalid_token")};let a=crypto.getRandomValues(new Uint8Array(16)),r=R(a);return await this.db.query("UPDATE auth.users SET reauthentication_token = $1, reauthentication_sent_at = NOW(), updated_at = NOW() WHERE id = $2",[r,t.payload.sub]),{error:null}}async enrollTOTP(e,t){await this.initialize();let a=await N(this.db,e);if(!a.valid||!a.payload)return null;let r=crypto.getRandomValues(new Uint8Array(20)),{base32:s}=await import("./base-ZCT74X2N.js"),i=s.encode(r),o=crypto.randomUUID(),u="nano-supabase",E=a.payload.email||a.payload.sub,_="otpauth://totp/"+u+":"+E+"?secret="+i+"&issuer="+u+"&algorithm=SHA1&digits=6&period=30",d="data:image/svg+xml;base64,"+btoa('<svg xmlns="http://www.w3.org/2000/svg"><text>'+_+"</text></svg>");return await this.db.query("INSERT INTO auth.mfa_factors (id, user_id, friendly_name, factor_type, status, secret, created_at, updated_at) VALUES ($1, $2, $3, 'totp', 'unverified', $4, NOW(), NOW())",[o,a.payload.sub,t??null,i]),{id:o,type:"totp",totp:{qr_code:d,secret:i,uri:_}}}async challengeTOTP(e,t){await this.initialize();let a=await N(this.db,e);if(!a.valid||!a.payload||!(await this.db.query("SELECT id FROM auth.mfa_factors WHERE id = $1 AND user_id = $2",[t,a.payload.sub])).rows[0])return null;let s=crypto.randomUUID(),i=Math.floor(Date.now()/1e3)+300;return await this.db.query("INSERT INTO auth.mfa_challenges (id, factor_id, created_at, ip_address) VALUES ($1, $2, NOW(), '127.0.0.1'::inet)",[s,t]),{id:s,expires_at:i}}async verifyTOTP(e,t,a,r){await this.initialize();let s=await N(this.db,e);if(!s.valid||!s.payload)return l("Invalid token",401,"invalid_token");let o=(await this.db.query("SELECT id, created_at, verified_at FROM auth.mfa_challenges WHERE id = $1 AND factor_id = $2",[a,t])).rows[0];if(!o||o.verified_at)return l("Invalid or expired challenge",400,"challenge_expired");if(Date.now()-new Date(o.created_at).getTime()>3e5)return l("Challenge expired",400,"challenge_expired");let _=(await this.db.query("SELECT secret FROM auth.mfa_factors WHERE id = $1 AND user_id = $2",[t,s.payload.sub])).rows[0];if(!_)return l("Factor not found",404,"factor_not_found");let{base32:d}=await import("./base-ZCT74X2N.js"),{verifyTOTP:T}=await import("./dist-BER53K2A.js"),h=d.decode(_.secret);if(!T(h,30,6,r))return l("Invalid TOTP code",422,"invalid_totp_code");await this.db.query("UPDATE auth.mfa_challenges SET verified_at = NOW() WHERE id = $1",[a]),await this.db.query("UPDATE auth.mfa_factors SET status = 'verified', updated_at = NOW() WHERE id = $1",[t]);let b=(await this.db.query("SELECT * FROM auth.users WHERE id = $1",[s.payload.sub])).rows[0];return b?this.signInAndCreateSession(b):l("User not found",404,"user_not_found")}async unenrollFactor(e,t){await this.initialize();let a=await N(this.db,e);return!a.valid||!a.payload?{error:m("Invalid token",401,"invalid_token")}:(await this.db.query("DELETE FROM auth.mfa_factors WHERE id = $1 AND user_id = $2",[t,a.payload.sub]),{error:null})}async adminListFactors(e){return await this.initialize(),(await this.db.query("SELECT id, factor_type, status, friendly_name, created_at FROM auth.mfa_factors WHERE user_id = $1 ORDER BY created_at",[e])).rows}async adminDeleteFactor(e,t){await this.initialize(),await this.db.query("DELETE FROM auth.mfa_factors WHERE id = $1 AND user_id = $2",[t,e])}async adminAuditLog(e=1,t=50){await this.initialize();let a=(e-1)*t,[r,s]=await Promise.all([this.db.query("SELECT id, payload, created_at, ip_address FROM auth.audit_log_entries ORDER BY created_at DESC LIMIT $1 OFFSET $2",[t,a]),this.db.query("SELECT count(*)::int AS total FROM auth.audit_log_entries")]);return{entries:r.rows,total:s.rows[0]?.total??0}}async verifyToken(e){return N(this.db,e)}};var S=class{store=new Map;async put(e,t,a){this.store.set(e,{data:t,metadata:a})}async get(e){return this.store.get(e)??null}async delete(e){return this.store.delete(e)}async deleteByPrefix(e){let t=0;for(let a of this.store.keys())a.startsWith(e)&&(this.store.delete(a),t++);return t}async exists(e){return this.store.has(e)}async copy(e,t){let a=this.store.get(e);return a?(this.store.set(t,{data:new Uint8Array(a.data),metadata:{...a.metadata}}),!0):!1}};var G=`
...
`;function X(n){return n.replace(/\+/g,"-").replace(/\//g,"_").replace(/=+$/,"")}function B(n){let e=n.replace(/-/g,"+").replace(/_/g,"/");for(;e.length%4;)e+="=";return e}var W=class{db;backend;initPromise=null;constructor(e,t){this.db=e,this.backend=t??new S}async initialize(){this.initPromise??=this.db.exec(G),await this.initPromise}getBackend(){return this.backend}async listBuckets(){return await this.initialize(),(await this.db.query("SELECT * FROM storage.buckets ORDER BY name")).rows}async getBucket(e){return await this.initialize(),(await this.db.query("SELECT * FROM storage.buckets WHERE id = $1",[e])).rows[0]??null}async createBucket(e){await this.initialize();let t=e.id??e.name,r=(await this.db.query(`INSERT INTO storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
       ...
       RETURNING *`,[t,e.name,e.public??!1,e.file_size_limit??null,e.allowed_mime_types??null])).rows[0];if(!r)throw new Error("Failed to create bucket");return r}async updateBucket(e,t){await this.initialize();let a=[],r=[],s=1;if(t.public!==void 0&&(a.push(`public = $${s++}`),r.push(t.public)),t.file_size_limit!==void 0&&(a.push(`file_size_limit = $${s++}`),r.push(t.file_size_limit)),t.allowed_mime_types!==void 0&&(a.push(`allowed_mime_types = $${s++}`),r.push(t.allowed_mime_types)),a.length===0){let u=await this.getBucket(e);if(!u)throw new Error("Bucket not found");return u}a.push("updated_at = now()"),r.push(e);let o=(await this.db.query(`UPDATE storage.buckets SET ${a.join(", ")} WHERE id = $${s} RETURNING *`,r)).rows[0];if(!o)throw new Error("Bucket not found");return o}async emptyBucket(e){await this.initialize(),await this.db.query("DELETE FROM storage.objects WHERE bucket_id = $1",[e]),await this.backend.deleteByPrefix(`${e}/`)}async deleteBucket(e){await this.initialize();let t=await this.db.query("SELECT count(*)::text as count FROM storage.objects WHERE bucket_id = $1",[e]);if(t.rows[0]&&parseInt(t.rows[0].count,10)>0)throw new Error("Bucket not empty");await this.db.query("DELETE FROM storage.buckets WHERE id = $1",[e])}async uploadObject(e,t,a,r,s){await this.initialize();let i=await this.getBucket(e);if(!i)throw new Error("Bucket not found");if(i.file_size_limit&&a.byteLength>i.file_size_limit)throw new Error(`File size ${a.byteLength} exceeds bucket limit of ${i.file_size_limit}`);if(i.allowed_mime_types&&i.allowed_mime_types.length>0&&!i.allowed_mime_types.some(h=>h.endsWith("/*")?r.startsWith(h.slice(0,-1)):r===h))throw new Error(`MIME type ${r} is not allowed in this bucket`);let o={eTag:`"${await this.computeETag(a)}"`,size:a.byteLength,mimetype:r,cacheControl:s?.cacheControl??"max-age=3600",lastModified:new Date().toISOString(),contentLength:a.byteLength,httpStatusCode:200},u=s?.upsert??!1,E;u?E=await this.db.query(`INSERT INTO storage.objects (bucket_id, name, owner_id, metadata, user_metadata, version)
         ...
         VALUES ($1, $2, $3, $4, $5, gen_random_uuid()::text)
         RETURNING *`,[e,t,s?.ownerId??null,JSON.stringify(o),s?.userMetadata?JSON.stringify(s.userMetadata):null]);let _=E.rows[0];if(!_)throw new Error("Failed to create object");let d=`${e}/${t}`;return await this.backend.put(d,a,{contentType:r,size:a.byteLength,cacheControl:s?.cacheControl}),_}async downloadObject(e,t){await this.initialize();let r=(await this.db.query(`UPDATE storage.objects
       ...
       DO UPDATE SET
         ...
         updated_at = now(),
         version = gen_random_uuid()::text
       RETURNING *`,[s,a,o.owner_id,JSON.stringify(o.metadata),o.user_metadata?JSON.stringify(o.user_metadata):null])).rows[0])throw new Error("Failed to copy object");let _=`${e}/${t}`,d=`${s}/${a}`;return await this.backend.copy(_,d),`${s}/${a}`}async createSignedUrl(e,t,a){await this.initialize();let r={bucket_id:e,object_name:t,exp:Math.floor(Date.now()/1e3)+a},i=(await this.db.query("SELECT auth.get_signing_key()")).rows[0]?.get_signing_key;if(!i)throw new Error("Signing key not available");let o=new TextEncoder,u=await crypto.subtle.importKey("raw",o.encode(i),{name:"HMAC",hash:"SHA-256"},!1,["sign"]),E=JSON.stringify(r),_=await crypto.subtle.sign("HMAC",u,o.encode(E)),d=X(btoa(E)),T=X(btoa(String.fromCharCode(...new Uint8Array(_))));return`${d}.${T}`}async verifySignedUrl(e){let t=e.split(".");if(t.length!==2)return null;let[a,r]=t;try{let s=atob(B(a??"")),i=JSON.parse(s);if(i.exp<Math.floor(Date.now()/1e3))return null;let o=await this.db.query("SELECT auth.get_signing_key()");if(!o.rows[0])return null;let u=new TextEncoder,E=await crypto.subtle.importKey("raw",u.encode(o.rows[0].get_signing_key),{name:"HMAC",hash:"SHA-256"},!1,["verify"]),_=Uint8Array.from(atob(B(r??"")),T=>T.charCodeAt(0));return await crypto.subtle.verify("HMAC",E,_,u.encode(s))?i:null}catch{return null}}async computeETag(e){try{let t=e.buffer.slice(e.byteOffset,e.byteOffset+e.byteLength),a=await crypto.subtle.digest("SHA-256",t),r=new Uint8Array(a);return Array.from(r.slice(0,8)).map(s=>s.toString(16).padStart(2,"0")).join("")}catch{return Math.random().toString(36).slice(2,18)}}};var C=(r=>(r[r.CRITICAL=0]="CRITICAL",r[r.HIGH=1]="HIGH",r[r.MEDIUM=2]="MEDIUM",r[r.LOW=3]="LOW",r))(C||{});var H={1:0,2:1,3:2},q=4,V=1e3,O=class{queues;heads;maxSize;agingThresholdMs;_size=0;lastAgingRun=0;constructor(e=1e3,t=5e3){this.maxSize=e,this.agingThresholdMs=t,this.queues=[[],[],[],[]],this.heads=[0,0,0,0]}enqueue(e){if(e.priority<0||e.priority>3)throw new Error(`Invalid priority: ${e.priority}`);if(this._size>=this.maxSize)throw new Error(`Queue is full (size: ${this._size}, max: ${this.maxSize})`);this.queues[e.priority].push(e),this._size++}dequeue(){let e=Date.now();e-this.lastAgingRun>V&&(this.applyAging(e),this.lastAgingRun=e);for(let t=0;t<q;t++){let a=t,r=this.queues[a],s=this.heads[a];if(s<r.length){let i=r[s]??null;return r[s]=null,this.heads[a]++,this._size--,this.heads[a]>r.length/2&&(this.queues[a]=r.slice(this.heads[a]),this.heads[a]=0),i}}return null}applyAging(e){for(let t of[1,2,3]){let a=this.queues[t],r=this.heads[t];if(r>=a.length)continue;let s=!1,i=this.queues[H[t]];for(let o=r;o<a.length;o++){let u=a[o];u!==void 0&&e-u.enqueuedAt>this.agingThresholdMs&&(u.priority=H[t],i.push(u),a[o]=null,s=!0)}if(s){let o=[];for(let u=r;u<a.length;u++){let E=a[u];E!=null&&o.push(E)}this.queues[t]=o,this.heads[t]=0}}}size(){return this._size}clear(){let e=[];for(let t=0;t<q;t++){let a=t,r=this.queues[a],s=this.heads[a];for(let i=s;i<r.length;i++){let o=r[i];o!==void 0&&e.push(o)}this.queues[a]=[],this.heads[a]=0}return this._size=0,e}sizeByPriority(){return{0:Math.max(0,this.queues[0].length-this.heads[0]),1:Math.max(0,this.queues[1].length-this.heads[1]),2:Math.max(0,this.queues[2].length-this.heads[2]),3:Math.max(0,this.queues[3].length-this.heads[3])}}};var Z=1e3,K=5e3,ee=5e3;function w(n){return n instanceof Error?n:new Error(String(n))}var Q=class n{db;queue;running=!1;config;wakeUp=null;processQueueDone=Promise.resolve();resolveProcessQueueDone=null;totalEnqueued=0;totalDequeued=0;totalTimedOut=0;totalErrors=0;waitTimeSum=0;waitTimeCount=0;nextQueryId=0;get pglite(){return this.db}constructor(e,t={}){this.db=e;let a=t.maxQueueSize??Z,r=t.agingThresholdMs??ee;this.queue=new O(a,r),this.config={maxQueueSize:a,defaultTimeout:t.defaultTimeout??K,agingThresholdMs:r}}static async create(e,t){let a=new n(e,t);return await a.start(),a}async start(){if(this.running)throw new Error("Pooler already started");this.running=!0,this.processQueueDone=new Promise(e=>{this.resolveProcessQueueDone=e}),setTimeout(()=>{this.processQueue().catch(e=>{console.error("Queue processor error:",e),this.running=!1,this.resolveProcessQueueDone?.()})},0),await new Promise(e=>setTimeout(e,0))}async stop(){if(!this.running)return;this.running=!1,this.wakeUp?.();let e=this.queue.clear();for(let t of e)t.reject(new Error("Pooler stopped"));await this.processQueueDone}async query(e,t,a=2,r){if(!this.running)throw new Error("Pooler is not running");return new Promise((s,i)=>{let o={kind:"sql",id:String(this.nextQueryId++),sql:e,params:t??[],priority:a,enqueuedAt:Date.now(),resolve:s,reject:i,timeoutMs:r??this.config.defaultTimeout};try{this.queue.enqueue(o),this.totalEnqueued++,this.wakeUp?.()}catch(u){i(w(u))}})}async transaction(e,t=2){if(!this.running)throw new Error("Pooler is not running");return new Promise((a,r)=>{let s={kind:"transaction",id:String(this.nextQueryId++),priority:t,enqueuedAt:Date.now(),resolve:a,reject:r,timeoutMs:this.config.defaultTimeout,transactionFn:e};try{this.queue.enqueue(s),this.totalEnqueued++,this.wakeUp?.()}catch(i){r(w(i))}})}metrics(){let e=this.waitTimeCount>0?this.waitTimeSum/this.waitTimeCount:0;return{totalEnqueued:this.totalEnqueued,totalDequeued:this.totalDequeued,totalTimedOut:this.totalTimedOut,totalErrors:this.totalErrors,currentSize:this.queue.size(),avgWaitTimeMs:e,sizeByPriority:this.queue.sizeByPriority()}}async[Symbol.asyncDispose](){await this.stop()}async processQueue(){for(;this.running;){let e=this.queue.dequeue();if(!e){await new Promise(t=>{this.wakeUp=t,setTimeout(()=>t(),1)}),this.wakeUp=null;continue}this.totalDequeued++,this.waitTimeSum+=Date.now()-e.enqueuedAt,this.waitTimeCount++;try{if(e.kind==="transaction"){let t=await this.runTransaction(e.transactionFn);e.resolve(t)}else{let t=await this.executeWithTimeout(e);e.resolve(t)}}catch(t){this.totalErrors++;let a=w(t);a.message==="Query timeout"&&this.totalTimedOut++,e.reject(a)}}this.resolveProcessQueueDone?.()}async runTransaction(e){return this.db.transaction(async t=>e((r,s)=>t.query(r,s)))}async executeWithTimeout(e){let t=e.timeoutMs??this.config.defaultTimeout,a=null,r=new Promise((i,o)=>{a=setTimeout(()=>{o(new Error("Query timeout"))},t)}),s=this.db.query(e.sql,[...e.params??[]]).finally(()=>{a!==null&&clearTimeout(a)});return Promise.race([s,r])}};export{k as a,D as b,U as c,A as d,N as e,ie as f,ne as g,P as h,M as i,z as j,Y as k,ue as l,x as m,S as n,G as o,W as p,C as q,O as r,Q as s};
//# sourceMappingURL=chunk-O3NROYSP.js.map
