...
async function main() {
  ...
  await db.insert(posts).values([
    { userId: 1, title: 'My First Post', content: 'Hello, world!', published: true },
    { userId: 1, title: 'Draft Post', content: 'Work in progress...', published: false },
    { userId: 2, title: "Bob's Post", content: 'Hi from Bob!', published: true },
  ])
  ...
  const updatedAlice: User[] = await db.select().from(users).where(eq(users.name, 'Alice'))
  ...
  console.log('  Typed insert result:', { id: inserted.id, title: inserted.title })
  console.log()
