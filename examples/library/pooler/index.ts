...
async function main() {
	...
	await Promise.all(
		Array.from({ length: 10 }, (_, i) =>
			pooler.query("INSERT INTO items (name, priority) VALUES ($1, 'MEDIUM')", [
				`item-${i}`,
			]),
		),
	);
	...
	await pooler.query("INSERT INTO items (name, priority) VALUES ($1, $2)", [
		"named",
		"HIGH",
	]);
	const found = await pooler.query("SELECT name FROM items WHERE name = $1", [
		"named",
	]);
	...
	const txCount = await pooler.query(
		"SELECT COUNT(*) AS n FROM items WHERE name = 'tx'",
	);
	console.log(
		`  Rows with name='tx' after rollback: ${txCount.rows[0]?.["n"]}\n`,
	);
