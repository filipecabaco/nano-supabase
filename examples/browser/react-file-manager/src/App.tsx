...
import { getSupabase, initDatabase } from "./db";
import "./App.css";
...
function NanoBoxLogo({ size = 28 }: { size?: number }) {
	return (
		...
		>
			...
			<path
				d="M32 4L4 18L32 32L60 18L32 4Z"
				fill="#3ECF8E"
				fillOpacity="0.25"
			/>
			<path
...
function App() {
	...
	useEffect(() => {
		files.forEach((f) => {
			if (f.mime_type.startsWith("image/")) loadPreview(f);
		});
	}, [files, loadPreview]);
		...
		<div className="min-h-screen flex flex-col">
			...
			<main className="flex-1 p-6">
				...
				>
					...
					/>
					<CloudArrowUpIcon
						className={`w-12 h-12 mx-auto mb-3 ${uploading ? "text-primary animate-pulse" : "text-muted-foreground"}`}
					/>
