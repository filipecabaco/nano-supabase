    ...
    if (url.pathname.startsWith("/admin/")) {
      ...
      if (slugMatch) {
        ...
        if (subpath === "/migrate" && req.method === "POST") {
          ...
          const { migrateDatabase } = await import("./cli-commands.ts");
            ...
            const result = await migrateDatabase(
              {
                querySource: <T = Record<string, unknown>>(
                  sql: string,
                  params?: unknown[],
                ) => nano.db.query<T>(sql, params),
                executeOnTarget: <T = Record<string, unknown>>(
                  sql: string,
                  params?: unknown[],
                ) => remote.query<T>(sql, params as T[]),
                downloadObject: async (bucketId, objectName) => {
                  const dlRes = await nano.localFetch(
                    new Request(
                      `http://localhost:54321/storage/v1/object/${bucketId}/${objectName}`,
                      {
                        headers: {
                ...
                uploadObject:
                  body.remoteUrl && body.remoteServiceRoleKey
                    ? async (bucketId, objectName, data, contentType) => {
                        const uploadRes = await fetch(
                          `${body.remoteUrl}/storage/v1/object/${bucketId}/${objectName}`,
                          {
                            method: "POST",
                            headers: {
              ...
              {
                skipSchema: body.skipSchema,
                skipAuth: body.skipAuth,
                skipData: body.skipData,
                skipStorage: body.skipStorage,
                dryRun: body.dryRun,
                migrationsDir: body.migrationsDir,
              },
            );
