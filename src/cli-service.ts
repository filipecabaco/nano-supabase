...
import type { StorageTransfer } from "./migrate-database.ts";
        ...
        if (subpath === "/migrate" && req.method === "POST") {
          ...
          try {
            ...
            const executeOnTarget = async (
              ...
              const r = await remote.query(sql, params);
              return {
                rows: r.rows as Record<string, unknown>[],
              };
            };

            let storageXfer: StorageTransfer | undefined;
              ...
              storageXfer = {
                ...
                upload: async (
                  ...
                  const uploadRes = await fetch(
                    `${body.remoteUrl}/storage/v1/object/${bucketId}/${name}`,
                    {
                      method: "POST",
                      headers: {
