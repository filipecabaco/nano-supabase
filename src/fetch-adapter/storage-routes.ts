...
export async function handleStorageRoute(
  ...
  try {
    ...
    if (pathname === "/storage/v1/upload/resumable" && method === "POST") {
      const meta = parseTusMetadata(request.headers.get("Upload-Metadata") ?? "");
      const bucketId = meta["bucketName"];
      const objectName = meta["objectName"];
      const contentType = meta["contentType"] ?? "application/octet-stream";
      const cacheControl = meta["cacheControl"];
      if (!bucketId || !objectName) return errorResponse("Missing Upload-Metadata", 400);

      const uploadLength = parseInt(request.headers.get("Upload-Length") ?? "0", 10);
      const upsert = request.headers.get("x-upsert") === "true";
      const uploadId = crypto.randomUUID();

      tusSessions.set(uploadId, {
        bucketId, objectName, contentType, cacheControl, upsert,
        ownerId: authCtx.userId, uploadLength, chunks: [], offset: 0, createdAt: Date.now(),
      });
    ...
    if (pathname === "/storage/v1/upload/resumable" && method === "PATCH") {
      ...
      if (session.offset >= session.uploadLength) {
        ...
        let pos = 0;
        for (const c of session.chunks) { combined.set(c, pos); pos += c.byteLength; }
        tusSessions.delete(uploadId);

        const obj = await storageHandler.uploadObject(
          session.bucketId, session.objectName, combined, session.contentType,
          { cacheControl: session.cacheControl, upsert: session.upsert, ownerId: session.ownerId },
        );

        return new Response(JSON.stringify({ Id: obj.id, Key: `${session.bucketId}/${session.objectName}` }), {
          status: 204,
          headers: {
            "Tus-Resumable": "1.0.0",
            "Upload-Offset": String(session.offset),
    ...
    if (signedUploadCreateMatch && method === "POST") {
      ...
      try {
        const uploadToken = await storageHandler.createSignedUrl(bucketId, objectPath, expiresIn);
        const signedUrl = `/storage/v1/object/upload/sign/${bucketId}/${objectPath}?token=${uploadToken}`;
        return jsonResponse({ signedUrl, token: uploadToken, path: `${bucketId}/${objectPath}` });
      } catch (err) {
    ...
    if (renderSignedMatch && method === "GET") {
      ...
      return await handleDownload(payload.bucket_id, payload.object_name, storageHandler);
    }
