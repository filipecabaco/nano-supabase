...
export function errorResponse(message: string, status = 400): Response {
  return jsonResponse({ statusCode: status.toString(), error: message, message }, status);
}

export function notSupported(description?: string): Response {
  return jsonResponse(
    { error: "not_supported", error_description: description ?? "Not supported in local mode" },
    400,
  );
}
