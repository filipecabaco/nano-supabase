...
export async function runStartMode(opts: {
  ...
  async function runAdvisors(
    ...
    const securityChecks: Array<{ query: string; map: (row: any) => unknown }> = [
      {
        query: `
    ...
    const performanceChecks: Array<{ query: string; map: (row: any) => unknown }> = [
      {
        query: `
      ...
      },
      {
        ...
        map: (row: { schema: string; table: string; index: string; duplicate_of: string }) => ({
