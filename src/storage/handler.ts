...
export class StorageHandler {
	...
	async initialize(): Promise<void> {
		this.initPromise ??= this.db.exec(STORAGE_SCHEMA_SQL);
	...
	async uploadObject(
		...
		if (bucket.allowed_mime_types && bucket.allowed_mime_types.length > 0) {
			...
			if (!allowed) {
				throw new Error(
					`MIME type ${contentType} is not allowed in this bucket`,
				);
			}
		}
		...
		let result: Awaited<ReturnType<typeof this.db.query<StorageObject>>>;
		if (upsert) {
	...
	async objectExists(bucketId: string, objectName: string): Promise<boolean> {
		await this.initialize();
	...
	async createSignedUrl(
		...
		const signingKey = keyResult.rows[0]?.get_signing_key;
		if (!signingKey) throw new Error("Signing key not available");
