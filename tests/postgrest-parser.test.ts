...
Deno.test('PostgREST Parser - SELECT with multiple filters', async () => {
  ...
  const result = parser.parseSelect('users', 'age=gte.18&status=eq.active&select=id,name')

  assertEquals(result.sql, 'SELECT "id", "name" FROM "users" WHERE "age" >= $1 AND "status" = $2')
...
Deno.test('PostgREST Parser - INSERT', async () => {
  ...
  assertEquals(result.sql, 'INSERT INTO "public"."users" ("age", "email", "name") VALUES ($1, $2, $3)')
...
Deno.test('PostgREST Parser - UPDATE', async () => {
  ...
  assertEquals(result.sql, 'UPDATE "public"."users" SET "age" = $1, "name" = $2 WHERE "id" = $3')
...
Deno.test('PostgREST Parser - RPC function call', async () => {
  ...
  assertEquals(result.sql, 'SELECT * FROM "public"."calculate_total"("order_id" := $1)')
...
Deno.test('PostgREST Parser - Generic HTTP request (GET)', async () => {
  ...
  const result = parser.parseRequest('GET', 'users', 'age=gte.18&select=id,name')

  assertEquals(result.sql, 'SELECT "id", "name" FROM "users" WHERE "age" >= $1')
