...
describe("Storage", () => {
  test("Upload, list, download, delete via supabase-js", async () => {
    ...
    const { error: deleteError } =
      await supabase.storage.deleteBucket("user-files");
  ...
  test("Folder navigation — list shows virtual folders and files at correct depth", async () => {
    ...
    await supabase.storage
      .from("nav-bucket")
      .upload("a/b/deep.txt", enc("deep"), { contentType: "text/plain" });
    await supabase.storage
      .from("nav-bucket")
      .upload("a/shallow.txt", enc("shallow"), { contentType: "text/plain" });
    await supabase.storage
      .from("nav-bucket")
      .upload("root.txt", enc("root"), { contentType: "text/plain" });
    ...
    const { data: inAB } = await supabase.storage
      .from("nav-bucket")
      .list("a/b");
    assertExists(inAB);
  ...
  test("Upload to nested folder and download back", async () => {
    ...
    const { error: e1 } = await supabase.storage
      .from("nested-bucket")
      .upload("a/b/c/deep.txt", enc("deep content"), {
        contentType: "text/plain",
      });
    ...
    const { error: e2 } = await supabase.storage
      .from("nested-bucket")
      .upload("a/b/mid.txt", enc("mid content"), { contentType: "text/plain" });
    assertEquals(e2, null);

    const { error: e3 } = await supabase.storage
      .from("nested-bucket")
      .upload("a/top.txt", enc("top content"), { contentType: "text/plain" });
    assertEquals(e3, null);

    const { data: deepFile, error: de } = await supabase.storage
      .from("nested-bucket")
  ...
  test("Move file between folders", async () => {
    ...
    await supabase.storage
      .from("move-bucket")
      .upload("folder-a/file.txt", enc("hello"), { contentType: "text/plain" });

    const { error: moveErr } = await supabase.storage
      .from("move-bucket")
      .move("folder-a/file.txt", "folder-b/file.txt");
    assertEquals(moveErr, null);

    const { data: atOld } = await supabase.storage
      .from("move-bucket")
      .list("folder-a");
    assertEquals(atOld?.length ?? 0, 0);

    const { data: atNew } = await supabase.storage
      .from("move-bucket")
      .list("folder-b");
    assertExists(atNew);
  ...
  test("Remove all files in a subfolder", async () => {
    ...
    await supabase.storage
      .from("rm-bucket")
      .upload("docs/a.txt", enc("a"), { contentType: "text/plain" });
    await supabase.storage
      .from("rm-bucket")
      .upload("docs/b.txt", enc("b"), { contentType: "text/plain" });
    await supabase.storage
      .from("rm-bucket")
      .upload("keep/c.txt", enc("c"), { contentType: "text/plain" });

    const { error: rmErr } = await supabase.storage
      .from("rm-bucket")
      .remove(["docs/a.txt", "docs/b.txt"]);
    assertEquals(rmErr, null);

    const { data: afterDocs } = await supabase.storage
      .from("rm-bucket")
      .list("docs");
    assertEquals(afterDocs?.length ?? 0, 0);

    const { data: afterKeep } = await supabase.storage
      .from("rm-bucket")
      .list("keep");
  ...
  test("getObjectInfo returns metadata for existing object and null for missing path", async () => {
    ...
    await supabase.storage
      .from("info-bucket")
      .upload("docs/readme.txt", new TextEncoder().encode("hello"), {
        contentType: "text/plain",
      });
  ...
  test("verifySignedUrl returns bucket and path for valid token and null for invalid/expired token", async () => {
    ...
    await supabase.storage
      .from("signed-bucket")
      .upload("file.txt", new TextEncoder().encode("content"), {
        contentType: "text/plain",
      });
