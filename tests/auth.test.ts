...
Deno.test(
  "Auth Schema - Multiple initialize calls are idempotent",
  async () => {
    const db = new PGlite({ extensions: { pgcrypto } });
    const authHandler = new AuthHandler(db);
...
Deno.test(
  "Auth UpdateUser - Returns current user when no updates",
  async () => {
    const db = new PGlite({ extensions: { pgcrypto } });
    const authHandler = new AuthHandler(db);
    await authHandler.initialize();
...
Deno.test(
  "Auth onAuthStateChange - Receives SIGNED_OUT on signout",
  async () => {
    const db = new PGlite({ extensions: { pgcrypto } });
    const authHandler = new AuthHandler(db);
    await authHandler.initialize();
...
Deno.test(
  "Crypto - createAccessToken generates valid JWT structure",
  async () => {
    const db = new PGlite({ extensions: { pgcrypto } });
    const authHandler = new AuthHandler(db);
    await authHandler.initialize();
