...
Deno.test('Pooler - Priority queue ordering', async () => {
  ...
  const promises = [
    pooler.query('SELECT 1 as val', [], QueryPriority.LOW).then(() => results.push('LOW')),
    pooler.query('SELECT 1 as val', [], QueryPriority.CRITICAL).then(() => results.push('CRITICAL')),
    pooler.query('SELECT 1 as val', [], QueryPriority.MEDIUM).then(() => results.push('MEDIUM')),
    pooler.query('SELECT 1 as val', [], QueryPriority.HIGH).then(() => results.push('HIGH')),
  ]
