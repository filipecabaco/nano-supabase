...
Deno.test(
  "Fetch Auth - POST /auth/v1/signup rejects missing email",
  async () => {
    const db = new PGlite({ extensions: { pgcrypto } });
    const { localFetch } = await createFetchAdapter({
      db,
...
Deno.test(
  "Fetch Auth - POST /auth/v1/signup rejects missing password",
  async () => {
    const db = new PGlite({ extensions: { pgcrypto } });
    const { localFetch } = await createFetchAdapter({
      db,
...
Deno.test(
  "Fetch Auth - POST /auth/v1/signup rejects duplicate email",
  async () => {
    const db = new PGlite({ extensions: { pgcrypto } });
    const { localFetch } = await createFetchAdapter({
      db,
...
Deno.test(
  "Fetch Auth - POST /auth/v1/token?grant_type=password succeeds",
  async () => {
    const db = new PGlite({ extensions: { pgcrypto } });
...
Deno.test(
  "Fetch Auth - POST /auth/v1/token rejects wrong password",
  async () => {
    const db = new PGlite({ extensions: { pgcrypto } });
    const { localFetch, authHandler } = await createFetchAdapter({
      db,
...
Deno.test(
  "Fetch Auth - POST /auth/v1/token?grant_type=refresh_token succeeds",
  async () => {
    const db = new PGlite({ extensions: { pgcrypto } });
    const { localFetch, authHandler } = await createFetchAdapter({
      db,
...
Deno.test(
  "Fetch Auth - POST /auth/v1/token rejects invalid refresh token",
  async () => {
    const db = new PGlite({ extensions: { pgcrypto } });
    const { localFetch } = await createFetchAdapter({
      db,
...
Deno.test(
  "Fetch Auth - POST /auth/v1/token rejects unsupported grant type",
  async () => {
    const db = new PGlite({ extensions: { pgcrypto } });
    const { localFetch } = await createFetchAdapter({
      db,
...
Deno.test(
  "Fetch Auth - GET /auth/v1/session returns null without auth",
  async () => {
    const db = new PGlite({ extensions: { pgcrypto } });
    const { localFetch } = await createFetchAdapter({
      db,
...
Deno.test(
  "Fetch Data - GET /rest/v1/table returns empty array for no results",
  async () => {
    const db = new PGlite({ extensions: { pgcrypto } });
...
Deno.test(
  "Fetch Data - POST /rest/v1/table with Prefer: return=representation",
  async () => {
    const db = new PGlite({ extensions: { pgcrypto } });
...
Deno.test(
  "Fetch Data - POST /rest/v1/table with Prefer: return=minimal",
  async () => {
    const db = new PGlite({ extensions: { pgcrypto } });
...
Deno.test(
  "Fetch Data - PATCH /rest/v1/table with return=representation",
  async () => {
    const db = new PGlite({ extensions: { pgcrypto } });
...
Deno.test(
  "Fetch Data - DELETE /rest/v1/table with return=representation",
  async () => {
    const db = new PGlite({ extensions: { pgcrypto } });
...
Deno.test(
  "Fetch Passthrough - Non-Supabase requests pass through",
  async () => {
    const db = new PGlite({ extensions: { pgcrypto } });
...
Deno.test(
  "Fetch RLS - Sets auth context for authenticated requests",
  async () => {
    const db = new PGlite({ extensions: { pgcrypto } });
...
Deno.test(
  "Fetch RLS - auth.uid() returns user ID in authenticated context",
  async () => {
    const db = new PGlite({ extensions: { pgcrypto } });
