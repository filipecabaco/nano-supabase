...
Deno.test('Fetch Auth - POST /auth/v1/signup succeeds', async () => {
  const db = new PGlite()
...
Deno.test('Fetch Auth - POST /auth/v1/signup with metadata', async () => {
  const db = new PGlite()
  const { localFetch } = await createFetchAdapter({
    db,
    supabaseUrl: SUPABASE_URL,
...
Deno.test('Fetch Auth - POST /auth/v1/signup rejects missing email', async () => {
  const db = new PGlite()
  const { localFetch } = await createFetchAdapter({
    db,
    supabaseUrl: SUPABASE_URL,
...
Deno.test('Fetch Auth - POST /auth/v1/signup rejects missing password', async () => {
  const db = new PGlite()
  const { localFetch } = await createFetchAdapter({
    db,
    supabaseUrl: SUPABASE_URL,
...
Deno.test('Fetch Auth - POST /auth/v1/signup rejects duplicate email', async () => {
  const db = new PGlite()
  const { localFetch } = await createFetchAdapter({
    db,
    supabaseUrl: SUPABASE_URL,
...
Deno.test('Fetch Auth - POST /auth/v1/token?grant_type=password succeeds', async () => {
  const db = new PGlite()
...
Deno.test('Fetch Auth - POST /auth/v1/token rejects wrong password', async () => {
  const db = new PGlite()
  const { localFetch, authHandler } = await createFetchAdapter({
    db,
    supabaseUrl: SUPABASE_URL,
...
Deno.test('Fetch Auth - POST /auth/v1/token?grant_type=refresh_token succeeds', async () => {
  const db = new PGlite()
  const { localFetch, authHandler } = await createFetchAdapter({
    db,
    supabaseUrl: SUPABASE_URL,
  ...
  const response = await localFetch(`${SUPABASE_URL}/auth/v1/token?grant_type=refresh_token`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
...
Deno.test('Fetch Auth - POST /auth/v1/token rejects invalid refresh token', async () => {
  const db = new PGlite()
  const { localFetch } = await createFetchAdapter({
    db,
    supabaseUrl: SUPABASE_URL,
...
Deno.test('Fetch Auth - POST /auth/v1/token rejects unsupported grant type', async () => {
  const db = new PGlite()
  const { localFetch } = await createFetchAdapter({
    db,
    supabaseUrl: SUPABASE_URL,
  ...
  const response = await localFetch(`${SUPABASE_URL}/auth/v1/token?grant_type=unsupported`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({}),
  })
...
Deno.test('Fetch Auth - GET /auth/v1/user returns user', async () => {
  const db = new PGlite()
  const { localFetch, authHandler } = await createFetchAdapter({
    db,
    supabaseUrl: SUPABASE_URL,
...
Deno.test('Fetch Auth - GET /auth/v1/user rejects missing auth', async () => {
  const db = new PGlite()
  const { localFetch } = await createFetchAdapter({
    db,
    supabaseUrl: SUPABASE_URL,
...
Deno.test('Fetch Auth - GET /auth/v1/user rejects invalid token', async () => {
  const db = new PGlite()
  const { localFetch } = await createFetchAdapter({
    db,
    supabaseUrl: SUPABASE_URL,
...
Deno.test('Fetch Auth - PUT /auth/v1/user updates user', async () => {
  const db = new PGlite()
  const { localFetch, authHandler } = await createFetchAdapter({
    db,
    supabaseUrl: SUPABASE_URL,
...
Deno.test('Fetch Auth - PUT /auth/v1/user rejects missing auth', async () => {
  const db = new PGlite()
  const { localFetch } = await createFetchAdapter({
    db,
    supabaseUrl: SUPABASE_URL,
...
Deno.test('Fetch Auth - POST /auth/v1/logout succeeds', async () => {
  const db = new PGlite()
  const { localFetch, authHandler } = await createFetchAdapter({
    db,
    supabaseUrl: SUPABASE_URL,
...
Deno.test('Fetch Auth - POST /auth/v1/logout works without token', async () => {
  const db = new PGlite()
  const { localFetch } = await createFetchAdapter({
    db,
    supabaseUrl: SUPABASE_URL,
...
Deno.test('Fetch Auth - GET /auth/v1/session returns session', async () => {
  const db = new PGlite()
  const { localFetch, authHandler } = await createFetchAdapter({
    db,
    supabaseUrl: SUPABASE_URL,
...
Deno.test('Fetch Auth - GET /auth/v1/session returns null without auth', async () => {
  const db = new PGlite()
  const { localFetch } = await createFetchAdapter({
    db,
    supabaseUrl: SUPABASE_URL,
...
Deno.test('Fetch Auth - Unknown auth endpoint returns 404', async () => {
  const db = new PGlite()
  const { localFetch } = await createFetchAdapter({
    db,
    supabaseUrl: SUPABASE_URL,
...
Deno.test('Fetch Data - GET /rest/v1/table returns all rows', async () => {
  const db = new PGlite()
...
Deno.test('Fetch Data - GET /rest/v1/table with column selection', async () => {
  const db = new PGlite()
  ...
  const response = await localFetch(`${SUPABASE_URL}/rest/v1/users?select=name,email`, {
    method: 'GET',
  })
...
Deno.test('Fetch Data - GET /rest/v1/table with filter', async () => {
  const db = new PGlite()
  ...
  const response = await localFetch(`${SUPABASE_URL}/rest/v1/users?select=*&age=gte.30`, {
    method: 'GET',
  })
...
Deno.test('Fetch Data - GET /rest/v1/table returns empty array for no results', async () => {
  const db = new PGlite()
...
Deno.test('Fetch Data - POST /rest/v1/table inserts row', async () => {
  const db = new PGlite()
...
Deno.test('Fetch Data - POST /rest/v1/table with Prefer: return=representation', async () => {
  const db = new PGlite()
...
Deno.test('Fetch Data - POST /rest/v1/table with Prefer: return=minimal', async () => {
  const db = new PGlite()
...
Deno.test('Fetch Data - PATCH /rest/v1/table updates rows', async () => {
  const db = new PGlite()
...
Deno.test('Fetch Data - PATCH /rest/v1/table with return=representation', async () => {
  const db = new PGlite()
...
Deno.test('Fetch Data - DELETE /rest/v1/table deletes rows', async () => {
  const db = new PGlite()
...
Deno.test('Fetch Data - DELETE /rest/v1/table with return=representation', async () => {
  const db = new PGlite()
...
Deno.test('Fetch Data - Handles invalid table', async () => {
  const db = new PGlite()
  ...
  const response = await localFetch(`${SUPABASE_URL}/rest/v1/nonexistent?select=*`, {
    method: 'GET',
  })
...
Deno.test('Fetch Data - Handles invalid path', async () => {
  const db = new PGlite()
...
Deno.test('Fetch Passthrough - Non-Supabase requests pass through', async () => {
  const db = new PGlite()
...
Deno.test('Fetch Passthrough - Edge functions pass through', async () => {
  const db = new PGlite()
...
Deno.test('Fetch RLS - Sets auth context for authenticated requests', async () => {
  const db = new PGlite()
...
Deno.test('Fetch RLS - auth.uid() returns user ID in authenticated context', async () => {
  const db = new PGlite()
  ...
  const insertResponse = await localFetch(`${SUPABASE_URL}/rest/v1/rpc/test_insert_auth_uid`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${accessToken}`,
...
Deno.test('Fetch RLS - Clears context for anonymous requests', async () => {
  const db = new PGlite()
  ...
  const response = await localFetch(`${SUPABASE_URL}/rest/v1/public_data?select=*`, {
    method: 'GET',
  })
...
Deno.test('Integration - Full auth + data flow', async () => {
  const db = new PGlite()
  ...
  const profileResponse = await localFetch(`${SUPABASE_URL}/rest/v1/profiles?select=*`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${accessToken}`,
  ...
  // 4. Update profile
  const updateResponse = await localFetch(`${SUPABASE_URL}/rest/v1/profiles?id=eq.${userId}`, {
    method: 'PATCH',
    headers: {
      'Content-Type': 'application/json',
      Prefer: 'return=representation',
...
Deno.test('Integration - Token refresh flow', async () => {
  ...
  // 2. Refresh token
  const refreshResponse = await localFetch(`${SUPABASE_URL}/auth/v1/token?grant_type=refresh_token`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      refresh_token: refreshToken,
...
Deno.test('Integration - Multiple users isolation', async () => {
  const db = new PGlite()
  ...
  const signup2 = await localFetch(`${SUPABASE_URL}/auth/v1/signup`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ email: 'user2@example.com', password: 'password2' }),
  })
  ...
  await db.query('INSERT INTO user_data (user_id, data) VALUES ($1, $2)', [userId1, 'User 1 data'])
  await db.query('INSERT INTO user_data (user_id, data) VALUES ($1, $2)', [userId2, 'User 2 data'])

  // User 1 fetches data
  const response1 = await localFetch(`${SUPABASE_URL}/rest/v1/user_data?select=*`, {
    method: 'GET',
  ...
  // User 2 fetches data
  const response2 = await localFetch(`${SUPABASE_URL}/rest/v1/user_data?select=*`, {
    method: 'GET',
...
Deno.test('Fetch Debug - Debug mode logs requests', async () => {
  const db = new PGlite()
