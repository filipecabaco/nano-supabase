...
Deno.test({ name: "createClient boots", ...testOptions, fn: async () => {
	const supabase = await createClient();
	assertExists(supabase);
} });

Deno.test({ name: "insert and select data", ...testOptions, fn: async () => {
  ...
  `);
	...
	await (supabase.from("items") as ReturnType<typeof supabase.from>).insert({ name: "deno-item" } as never);

	const { data, error: selectError } = await supabase.from("items").select("*");
...
Deno.test({ name: "auth signup and signin", ...testOptions, fn: async () => {
	const supabase = await createClient();
...
Deno.test({ name: "RLS blocks unauthenticated access", ...testOptions, fn: async () => {
	const nano = await nanoSupabase();
	await nano.db.exec(`
