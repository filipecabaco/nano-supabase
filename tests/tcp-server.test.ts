...
describe("PGliteTCPServer", () => {
  ...
  test("parameterized query via extended protocol", async () => {
    const { pool, teardown } = await setup();
    try {
      await pool.query("CREATE TABLE products (id SERIAL PRIMARY KEY, price NUMERIC)");
      await pool.query("INSERT INTO products (price) VALUES (9.99), (19.99), (29.99)");
      const res = await pool.query("SELECT price FROM products WHERE price > $1 ORDER BY price", [15]);
      assertEquals(res.rows.length, 2);
    } finally {
      await teardown();
    }
  ...
  test("JSONB columns are returned as parsed objects", async () => {
    ...
    try {
      ...
      await pool.query(`INSERT INTO docs VALUES ('{"key": "value", "num": 42}')`);
  ...
  test("multi-statement simple query returns last result", async () => {
    ...
    try {
      ...
      const res = await pool.query("SET statement_timeout = '5s'; SELECT val FROM things");
