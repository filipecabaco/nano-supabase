...
describe("LEAN_POSTGRES_OPTIONS", () => {
	test("instances start and execute queries with lean postgres options", async () => {
		await using nano = await nanoSupabase({ postgresOptions: LEAN_POSTGRES_OPTIONS });
	...
	test("data queries work with lean options", async () => {
		await using nano = await nanoSupabase({ postgresOptions: LEAN_POSTGRES_OPTIONS });
		await nano.db.exec("CREATE TABLE IF NOT EXISTS items (id SERIAL PRIMARY KEY, name TEXT)");
		const supabase = nano.createClient();
...
describe("shared PostgrestParser", () => {
	test("second instance reuses provided parser and skips schema introspection", async () => {
		...
		await nano1.db.exec("CREATE TABLE IF NOT EXISTS things (id SERIAL PRIMARY KEY, label TEXT)");

		const sharedParser = new PostgrestParser();

		await using nano2 = await nanoSupabase({ parser: sharedParser });
		await nano2.db.exec("CREATE TABLE IF NOT EXISTS things (id SERIAL PRIMARY KEY, label TEXT)");

		const supabase2 = nano2.createClient();
		const { error } = await supabase2.from("things").insert({ label: "shared" });
		expect(error).toBeNull();
	...
	test("shared parser + lean options combination works", async () => {
		...
		await nano.db.exec("CREATE TABLE IF NOT EXISTS entries (id SERIAL PRIMARY KEY, val TEXT)");
		const supabase = nano.createClient();
...
describe("multiple instances", () => {
	test("5 instances can start concurrently and serve queries", async () => {
		const instances = await Promise.all(
			Array.from({ length: 5 }, () => nanoSupabase({ postgresOptions: LEAN_POSTGRES_OPTIONS }))
		);

		await Promise.all(instances.map(async (nano, i) => {
			await nano.db.exec(`CREATE TABLE IF NOT EXISTS t${i} (id SERIAL PRIMARY KEY, v TEXT)`);
			const supabase = nano.createClient();
